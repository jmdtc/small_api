  ...
  const dimensionsChecked = Object.keys(datatypes).map(key => {
    return {
      ...
      wrongType: Object.keys(queryObject).filter(queryKey => {
        const formattedParamsName =
          key === "integer" ?
          removeBrackets(queryKey): queryKey
        console.log(formattedParamsName);
        if (!Object.keys(datatypes).some(type => datatypes[type].includes(formattedParamsName))) return true
        if (!datatypes[key].includes(formattedParamsName)) return false
        switch (key) {
          case "integer":
